...
#+BEGIN_EXAMPLE yaml
  ...
  - data: [name of a data set in _data]
    ...
    dir_expr: [a Ruby expression to generate the dir (alternative to dir)]
...
- ~dir_expr~ is an optional Ruby expression used to generate the directory where the pages will be generated. The
  expression can reference fields of the data being read using the ~record~
  hash (e.g., ~record['first_name'] + "_" + record['last_name']~)
  /Optional: used in  alternative to ~dir~. If set it overrides ~dir~/
